...
class Node:
    ...
    def calc_bin_entropy(b, att):
        return entropy(filter(lambda x: x=b, att))

        def calacGainForAtt(self, colms):

            gains = {}
            sum_of_bins_etropy = 0

            class_entropy = entropy(colms[len(colms - 1)].values())

            for att in colms:
                for b in att:
                    bin_entropy = calc_bin_entropy(b, att)
                    sum_of_bins_etropy = bin_entropy + sum_of_bins_etropy
        ...
        def id3Tree(self, exemples):
            ...
            while root == None:
                ...
                max = gains.keys()[max(gains.values())]
